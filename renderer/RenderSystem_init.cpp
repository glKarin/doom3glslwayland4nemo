...
#ifdef _HARMATTAN_OPTMIZE
idCVar h_rCompressTexture( "h_rCompressTexture", "0", CVAR_RENDERER | CVAR_INTEGER, "[Harmattan] - Compressing texture when loading texture to GPU memory: 0 to disable, less than 0 to means compress level, others means compressed size." );
idCVar h_tRenderer( "h_tRenderer", "0", CVAR_RENDERER | CVAR_INTEGER, "[Harmattan] - UNUSED, only for testing renderer, every bit is a state." );
idCVar h_rNoLighting( "h_rNoLighting", "1", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE, "[Harmattan] - Rendering without lighting: 0 / 1" );
#endif
