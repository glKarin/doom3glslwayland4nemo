...
void karinNewVKB(float x, float y, float z, float w, float h)
{
	...
	Mesa_glLoadIdentity(modelview_matrix);
...
void karinRenderVKBToFBO(void)
{
	...
	Mesa_glLoadIdentity(projection_matrix);
	Mesa_glOrtho(NULL, 0.0, the_vkb.height, 0.0, the_vkb.width, -1.0, 1.0, projection_matrix);
	Mesa_glTranslate(projection_matrix, 0, the_vkb.width, 0, projection_matrix);
	float m[16];
	memcpy(m, projection_matrix, sizeof(float) * 16);
	Mesa_glRotate(m, -90, 0, 0, 1, projection_matrix);	    // for sailfish
...
void karinRenderVKB(void)
{
	...
	karinBeginRender2D(the_vkb.height, the_vkb.width);
	{
		int i;
		int count = TOTAL_VKB_COUNT;
		for(i = 0; i < count; i++)
...
void karinRenderVKBJoystick(const virtual_joystick *b, const texture tex[])
{
	...
	{
		Mesa_glLoadIdentity(modelview_matrix);
		Mesa_glTranslate(modelview_matrix, cx + b->pos_x, cy + b->pos_y, 0.0f, modelview_matrix);
		karinSetupMatrix();
...
void karinRenderVKBCursor(const virtual_cursor *b, const texture tex[])
{
	...
	{
		glBindBuffer(GL_ARRAY_BUFFER, b->base.buffers[Texture_Coord]);
		karinSetupTexCoord((float *)NULL + 16);
		glBindBuffer(GL_ARRAY_BUFFER, b->base.buffers[Position_Coord]);
		karinSetupVertex((float *)NULL + 8);
